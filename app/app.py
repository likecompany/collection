...
from orm import CollectionModel
...
def create_application() -> FastAPI:
    ...
    application = FastAPI(
        title="like.company.poker.collection",
    ...
    application.include_router(api_router, prefix="/poker.collection", tags=["poker.collection"])
    ...
    def create_admin_panel() -> None:
        ...
        if server_settings.DEBUG:
            admin = SQLAlchemyAdmin(
                base_url="/poker.collection",
                engine=engine,
                debug=True,
            )

            admin.add_view(SQLAlchemyModelView(CollectionModel))
